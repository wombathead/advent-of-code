...
(defun advent-08b (filename)
  (flet ((build-number-map (pattern number-codes code-numbers)
    ...
    (loop with input = (get-file filename)
          for code-numbers = (make-hash-table :test 'equal)
          for number-codes = (make-array (list 10))
          for line in input
          for (signal-pattern output-value) = (str:split " | " line)
          for pattern = (str:words signal-pattern)
          do
          (build-number-map pattern number-codes code-numbers)
